...
class Usuario(database.Model, UserMixin, UniversalMixin):
    ...
    @property
    def nombre(self):
        """Junta nombres, apellido_primero y apellido materno"""
    ...
    def launch_task(self, comando, mensaje, *args, **kwargs) -> Tarea:
        """Lanzar una tarea en segundo plano"""
        job = current_app.task_queue.enqueue(f"pjecz_perseo_flask.blueprints.{comando}", *args, **kwargs)
        tarea = Tarea(id=job.get_id(), comando=comando, mensaje=mensaje, usuario=self)
        tarea.save()
        return tarea
